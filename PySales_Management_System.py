...
delete_entry.insert(0, '삭제하실 상품을 입력해주세요')

# create the labels
name_label = tk.Label(root_window, text="상품명")
id_label = tk.Label(root_window, text="상품  ID")
quantity_sold_label = tk.Label(root_window, text="판매량")
quantity_left_label = tk.Label(root_window, text="남은 수량")
price_label = tk.Label(root_window, text="가격")
today_sales_quantity_label = tk.Label(root_window, text="오늘의 판매 수량)")
today_sales_total_price_label = tk.Label(root_window, text="오늘의 판매 수량")
week_sales_quantity_label = tk.Label(root_window, text="주간 판매 수량")
week_sales_total_price_label = tk.Label(root_window, text="주간 매출액")
month_sales_quantity_label = tk.Label(root_window, text="월 판매 수량")
month_sales_total_price_label = tk.Label(root_window, text="월 매출액")

product_list_label = tk.Label(root_window, text='상품리스트', font=(30), borderwidth=1, relief="solid", width=50)
product_list_label.grid(row=12, column=0, columnspan=50, pady=20)

pdt_name = tk.Label(root_window, text='상품명', borderwidth=1, relief="solid")
pdt_id = tk.Label(root_window, text='상품 ID', borderwidth=1, relief="solid")
pdt_price = tk.Label(root_window, text='가격', borderwidth=1, relief="solid")
pdt_sales_quantity = tk.Label(root_window, text="오늘 판매 수량", borderwidth=1, relief="solid")
...
# creating buttons
update_button = tk.Button(text='업데이트', width=20, height=2, command=update)
delete_button = tk.Button(text='삭제', width=20,height=2, command=delete_product)
add_button = tk.Button(text='추가', width=20, height=2, command=add_product)
add_button.config(state='disabled')
calculate_button = tk.Button(text='확정', width=25, height=2, command=calculate)
refresh_button = tk.Button(text='초기화', width=25, height=2, command=Refresh)
